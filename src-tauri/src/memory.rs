//! Memory CRUD. Memories are short text records tagged with type/project/importance.
//! The "code" type is reserved for tree-sitter indexed code chunks (see ingest.rs).
...
pub struct RememberInput {
    ...
    pub end_line: Option<i64>,
    pub language: Option<String>,
    pub supersedes: Option<String>, // uid of memory this replaces
}
...
pub fn remember(state: &AppState, input: RememberInput) -> BiResult<Memory> {
    ...
    state.db.write(|tx| {
        // Mark supersedes target as superseded.
...
pub fn search(
    ...
    // Optional type filter: build allowlist of uids in this project with that type.
    ...
    for h in hits {
            ...
            .ok()
        {
            ...
            db::log_activity(
                ...
                "read",
                Some(&h.uid),
                Some(&serde_json::json!({"score": h.score, "query": query})),
            )?;
            out.push(MemoryWithScore { memory: mem, score: h.score });
        }
    }
    Ok(out)
}
